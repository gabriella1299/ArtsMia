...
public class ArtsmiaDAO {
	...
	public void  listObjects(Map<Integer,ArtObject> map) {
		...
		try {
			...
			while (res.next()) {
				if(!map.containsKey(res.getInt("object_id"))) {//cosi gli oggetti vengono creati una sola volta
	...
	public int getPeso(ArtObject a1, ArtObject a2) {
		...
		try {
			...
			int peso=0;
			if(res.next()){//almeno un risultato ci sara perche abbiamo count
	...
	public List<Adiacenza> getAdiacenze() {//creiamo classe apposta per la query piu'complicata
		...
		try {
			...
			while (res.next()){//almeno un risultato ci sara perche abbiamo count
